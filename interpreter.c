...
int main(int argc,char*argv[]){
    ...
    int p1=putString_RandomAcess_MEMORY("1st");
    int p2=putString_RandomAcess_MEMORY("2nd");

    int s1=find_RandomAccess_MEMORY();
    prompt('\n',p1,s1,_MEMORY_X_SIZE_);
    
    int s2=find_RandomAccess_MEMORY();
    prompt('\n',p2,s2,_MEMORY_X_SIZE_);

    concatStrings_PP_MEMORY(s1,s2,0);

    printString_MEMORY(s1,1);

    int i=find_RandomAccess_MEMORY();
    copyString_MEMORY(s1,i);
...
int compareStrings_PS_MEMORY(int a,char b[_MEMORY_X_SIZE_]){
    for(int i=0;i<_MEMORY_X_SIZE_&&
    (getChar_MEMORY(a,i)!='\0'||b[i]!='\0');i++)#include <stdio.h>
#include <stdlib.h>
#include <string.h>
